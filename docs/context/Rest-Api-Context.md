...
- `GET /api/v1/tickets/recent?project_key=` (project optional → all recent for
  the key's connection).
- `200` → `{ "items": [{ "issueKey", "title", "url", "createdAt" }], "source": "cache" | "jira" }`
- Same Zod project validation; errors as above.
